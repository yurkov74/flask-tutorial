...
current_app is another special object that points to the Flask application \
  ...
  application object when writing the rest of the code. get_db will be called \
  when the application has been created and is handling a request, so \
...
def close_db(e=None):
    """
    checks if a connection was created by checking if g.db was set. If the \
      connection exists, it is closed. Further down you will tell your \
...
def init_db_command():
    ...
    click.command() defines a command line command called init-db that calls \
      the init_db function and shows a success message to the user. You can \
      read Command Line Interface to learn more about writing commands.
...
The close_db and init_db_command functions need to be registered with the \
  ...
  However, since you’re using a factory function, that instance isn’t \
  available when writing the functions. Instead, write a function that takes \
...
def init_app(app):
    ...
    """
    app.cli.add_command() adds a new command that could be called with the \
      flask command.
    """
