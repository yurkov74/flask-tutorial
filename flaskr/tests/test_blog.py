"""
Unit tests for blog functionality.

All the blog views use the auth fixture, call all auth.login() and subsequent \
  requests from the client will be logged in as the test user.
"""
import pytest
...
def test_index(client, auth):
    ...
    The index view should display information about the post that was added \
      with the test data. When logged in as the author, there should be a \
    ...
    assert b'by test on 2024-03-01' in response.data
...
"""
The create and update views should render and return a 200 OK status for a \
  GET request. When valid data is sent in a POST request, create should \
  insert the new post data into the database, and update should modify the \
  existing data. Both pages should show an error message on invalid data.
"""
